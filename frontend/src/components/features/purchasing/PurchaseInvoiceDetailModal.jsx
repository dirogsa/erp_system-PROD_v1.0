...
import { createDebitNote } from '../../../services/api';
import { formatCurrency, formatDateTime, getPaymentStatusVariant } from '../../../utils/formatters';

const PurchaseInvoiceDetailModal = ({ invoice, visible, onClose, onUpdate }) => {
    const [isDebitNoteModalOpen, setDebitNoteModalOpen] = useState(false);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [submitError, setSubmitError] = useState(null);

    const handleCreateDebitNote = async (data) => {
        setIsSubmitting(true);
        setSubmitError(null);
        try {
            await createDebitNote(invoice.id, data);
            setDebitNoteModalOpen(false);
            onUpdate(); // Refresh data
        } catch (error) {
            setSubmitError(error.response?.data?.detail || 'Error al crear la nota de débito.');
        } finally {
    ...
    const pendingAmount = invoice.total_amount - (invoice.amount_paid || 0) - (invoice.debit_applied || 0);

    return (
        <>
            <Modal isOpen={visible} onClose={onClose} title={`Factura de Compra: ${invoice.invoice_number}`} size="3xl">
                ...
                <div className="mb-4">
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {invoice.items.map((item, index) => (
                                <tr key={index} className="border-b border-gray-800">
                                    ...
                                    <td className="text-right p-2">{formatCurrency(item.quantity * item.unit_cost)}</td>
                                </tr>
                            ))}
                        </tbody>
                        <tfoot className="font-bold">
                            ...
                            <tr>
                                <td colSpan="3" className="text-right p-2 text-yellow-400">Monto con ND (Débito):</td>
                                <td className="text-right p-2 text-yellow-400">-{formatCurrency(invoice.debit_applied || 0)}</td>
                            </tr>
                             <tr>
                ...
                {/* Applied Debit Notes Section */}
                {invoice.debit_note_ids && invoice.debit_note_ids.length > 0 && (
                     <div className="mb-4">
                        <h3 className="font-bold mb-2">Notas de Débito Aplicadas</h3>
                        <div className="bg-gray-800 p-3 rounded-lg">
                            <ul className="list-disc pl-5">
                                {invoice.debit_note_ids.map(dn_id => (
                                    <li key={dn_id} className="text-sm">ID: {dn_id}</li>
                                ))}
                            </ul>
                        </div>
                    </div>
                ...
                <div className="flex justify-end gap-2">
                    ...
                    <Button variant="primary" onClick={() => setDebitNoteModalOpen(true)}>Crear Nota de Débito</Button>
                </div>
            </Modal>

            {/* Debit Note Creation Modal */}
            <Modal isOpen={isDebitNoteModalOpen} onClose={() => setDebitNoteModalOpen(false)} title="Crear Nota de Débito" size="2xl">
                {submitError && <Alert variant="error" className="mb-4">{submitError}</Alert>}
                {isSubmitting ? (
                    <Loading text="Creando nota de débito..." />
                ) : (
                    <DebitNoteForm 
                        invoice={invoice} 
                        onSubmit={handleCreateDebitNote} 
                        onCancel={() => setDebitNoteModalOpen(false)}
                    />
                )}
            </Modal>
        </>
